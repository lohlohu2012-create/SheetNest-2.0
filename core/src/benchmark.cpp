...
BenchmarkCase runCase(
    ...
    // Each benchmark mode starts with the same cold NFP cache. Without this,
    // the optimized case could inherit entries generated by the baseline and
    // its elapsed time would no longer measure an equivalent workload.
    nfp::clearCache();
    ...
    benchmark.nfpTimeoutFallbacks = result.stats.nfpTimeoutFallbacks;
